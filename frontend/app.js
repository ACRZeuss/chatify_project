...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const loadRooms = async () => {
    try {
      ...
      if (rooms.length === 0) {
        roomList.innerHTML =
          "<p>Henüz hiç sohbet odası yok. İlk odayı sen oluştur!</p>";
      } else {
        rooms.forEach((room) => {
  ...
  const loadMessageHistory = async (slug) => {
    chatLog.innerHTML = ""; // Önceki mesajları temizle
    try {
      const messages = await apiRequest(
        `http://127.0.0.1:8000/api/rooms/${slug}/messages/`
      );
      messages.forEach((msg) => {
        chatLog.innerHTML += `<div><b>${msg.author_username}:</b> ${msg.content}</div>`;
      });
      chatLog.scrollTop = chatLog.scrollHeight; // Sohbeti en aşağı kaydır
    } catch (error) {
      console.error("Mesaj geçmişi yüklenemedi:", error);
    }
  };

  const enterChatRoom = async (slug, name) => {
    // Fonksiyonu async yap
    showView(chatContainer);
    roomNameHeader.textContent = `Sohbet Odası: ${name}`;

    // Odaya girer girmez mesaj geçmişini yükle
    await loadMessageHistory(slug);
  ...
  loginForm.addEventListener("submit", async (e) => {
    ...
    try {
      await apiRequest("http://127.0.0.1:8000/api/auth/login/", "POST", {
        username,
        password,
      });
